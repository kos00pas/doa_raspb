...
class Classification_Window:
    ...
    def make_mfcc(self):
            ...
            #print(f"Predictions: {class_predictions[0]}")
            prediction = class_predictions[0]
            self.main_window.prediction_label.config(font="bold", text=f'Prediction: {prediction}')
            print(prediction)
            ...
            self.DATA.Save_signal_csv(audio, save_dir)
            self.DATA.Save_prediction_csv(np.array([prediction]), save_dir)  # Save prediction as numpy array

            time_end = time.time()
            elapsed_time_mfcc = time_end - time_start
            #print(f'Time taken for MFCC computation: {elapsed_time_mfcc} seconds')
