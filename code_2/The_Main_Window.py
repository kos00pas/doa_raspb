...
class Main_Window(tk.Tk):
    def __init__(self, gui_manager, database):
        ...
        self.is_mfcc_running = False  
        self.last_mfcc_start_time = 0  
    ...
    def setup_control_frames(self):
        ...
        self.prediction_label = tk.Label(self.down_control_frame, text="Prediction: ", width=20, font="bold")
        self.prediction_label.pack(side=tk.RIGHT, padx=5, pady=5)
    ...
    def make_mfcc(self):
        ...
        """
        if self.is_mfcc_running:
            self.is_mfcc_running = False
            self.make_mfcc_button.config(text="Mfcc     ", bg="light yellow")
            self.close_mfcc_event.set()
        else:
            self.is_mfcc_running = True
            self.make_mfcc_button.config(text="Stop Mfcc", bg="magenta")
            self.close_mfcc_event.clear()
            self.run_mfcc_repeatedly()

    def run_mfcc_repeatedly(self):
        """
            Scope:      Run MFCC repeatedly until stopped
        """
        def thread_mfcc_function():
            while self.is_mfcc_running and not self.close_mfcc_event.is_set() and self.DATA.recording:
                current_time = time.time() * 1000  # Get current time in milliseconds
                if current_time - self.last_mfcc_start_time >= self.DATA.cnn_time:
                    self.last_mfcc_start_time = current_time
